...
//echo "SELECT member_id, password, member_role FROM ".DATABASE_USERS." WHERE member_id = " . $cDB->EscTxt($user) . " AND (password=sha(". $cDB->EscTxt($pass) .") OR password=". $cDB->EscTxt($pass) .") and status = 'A';";
	...
	function UserLoginLogout() {
		...
		if ($this->IsLoggedOn())
		{
			...
			$output = "<A HREF=\"member_login.php\" title=\"$lng_login (".DATABASE_NAME.")\">".$lng_login."</A>&nbsp;&nbsp;&nbsp;";
		}
  ...
  // Show a map generated by Google Maps
  $map_address = $this->person[0]->address_street1;
  $map_city    = $this->person[0]->address_city;      
  $map_url = "https://maps.google.nl/maps?hl=nl".
             "&amp;q=".$map_address.",+".$map_city.        // Location
             "&amp;ie=UTF8&amp;hq=".                                      
             "&amp;hnear=".$map_address.",+".$map_city.   // Shows a marker
             "&amp;t=m&amp;z=14".                                           // Zoom level
             "&amp;iwloc=near";                                             // Hides the ugly information balloon
              
  $output .= "<br>".
             "<iframe width=\"300\" height=\"200\" frameborder=\"1\" scrolling=\"no\"".
             "  marginheight=\"0\" marginwidth=\"0\" ".
             "  src=\"".$map_url."&amp;output=embed\">". // 'embed' is really needed!
             "</iframe>".
             "<br />".
             "<small><a href=\"".$map_url."\" target=\"_blank\">Grotere kaart weergeven</a></small>".
             "<br>";                         
	...
	if (SOC_NETWORK_FIELDS==true) {	
		...
		$pAbout = (!stripslashes($this->person[0]->about_me)) ? '<em>'.
		            $lng_no_description_supplied.'.</em>' : stripslashes(nl2br($this->person[0]->about_me));
		
		$output .= "<STRONG>".$lng_age.":</STRONG> ".$pAge."<br>";
		$output .= "<STRONG>".$lng_sex.":</STRONG> ".$pSex."<br>";
		$output .= "<br>".$pAbout;		
...
//		$form->addElement("select", "member_to", "...", $name_list->MakeNameArray());
	...
	function EmailListingUpdates($interval) {
		...
		if($email_text == "") // (!=$email_text) replaced by ($email_text == "") - by ejkv
