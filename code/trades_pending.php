...
$member = $cUser; // added by ejkv
$member_logged_in = $member->member_id; // added by ejkv

$pending = new cTradesPending($member_logged_in); // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
...
function doTrade($t) {
	...
	if ($t["typ"]=='T')
		$member_to->LoadMember($_SESSION["user_login"]); // NOT replaced $_SESSION["user_login"] by $member_logged_in - ejkv
	else
	...
	if ($t["typ"]=='T')
		...
		$member->LoadMember($_SESSION["user_login"]); // NOT replaced $_SESSION["user_login"] by $member_logged_in - ejkv
...
switch($_REQUEST["action"]) {
	...
	case("resend"): 
		...
		if ($result && mysql_num_rows($result)>0) { // Trade Exists
			...
			if ($row["member_id_from"]!=$member_logged_in) { // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
	...
	case("reject"): 
		...
		if ($result && mysql_num_rows($result)>0) { // Trade Exists
			...
			// Do we have permission to act on this trade?
			if ($row["member_id_to"]!=$member_logged_in) { // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
			...
			if ($row["typ"]=='T' && $row["member_id_to"]==$member_logged_in) { // We want to reject the payment!
				 // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
	...
	case("remove"): 
		...
		if ($result && mysql_num_rows($result)>0) { // Trade Exists
			...
			// Do we have permission to act on this trade?
			if ($row["member_id_from"]!=$member_logged_in && $row["member_id_to"]!=$member_logged_in) { // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
				...
				if ($row["typ"]=='T') { // Payment - we are confirming receipt of incoming
					...
					if ($row["member_id_to"]!=$member_logged_in) // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
						...
						$list .= "<em>".$lng_no_permission_to_confirm_trade."</em>";
							...
							/*
							$goingFrom = $_SESSION["user_login"]; // NOT replaced $_SESSION["user_login"] by $member_logged_in - ejkv
	...
	case("incoming"):
		...
		*/
		...
		$q = "SELECT * FROM trades_pending where member_id_to=".$cDB->EscTxt($member_logged_in)." and typ='T' and member_to_decision = 1"; // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
	...
	case("outgoing"):
		...
		$q = "SELECT * FROM trades_pending where member_id_to=".$cDB->EscTxt($member_logged_in)." and typ='I' and member_to_decision = 1"; // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
	...
	case("payments_sent"):
		...
		*/
		...
		$q = "SELECT * FROM trades_pending where member_id_from=".$cDB->EscTxt($member_logged_in)." and typ='T' and member_from_decision = 1"; // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
	...
	case("invoices_sent"):
		...
		*/
		...
		$q = "SELECT * FROM trades_pending where member_id_from=".$cDB->EscTxt($member_logged_in)." and typ='I' and member_from_decision = 1"; // replaced $_SESSION["user_login"] by $member_logged_in - ejkv
