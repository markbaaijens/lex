...
class cState
{
	...
	function LoadState($id) {
		global $cDB, $cErr, $lng_error_access_state_code, $lng_please_try_again_later;
		...
		if($row = mysql_fetch_array($query)) {		
			...
			$cErr->Error($lng_error_access_state_code." '".$id."'.  ".$lng_please_try_again_later.".");
...
// DeleteState not used, state only to be changed, else there should be checked if the state is still used - ejkv
	function DeleteState() {
		global $cDB, $cErr, $lng_error_delete_state_code, $lng_please_try_again_later;

		$cErr->Error($lng_error_delete_state_code." '".$id."'. FUNCTION NOT USED."); // function not to be used
		...
		if(mysql_affected_rows() == 1) {
			...
			$cErr->Error($lng_error_delete_state_code." '".$id."'. ".$lng_please_try_again_later.".");
			include("redirect.php");
		}
	}
...
class cStateList {
	...
	function LoadStateList($redirect=false) {	
		global $cDB, $cErr, $lng_error_access_state_record, $lng_please_try_again_later;
		...
		if($i == 0) {
			if ($redirect) {
				$cErr->Error($lng_error_access_state_record.".  ".$lng_please_try_again_later.".");
				include("redirect.php");			
			} else {
				return false;
			}
